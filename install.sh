...
VERSION="${ADAPTIVE_LEARNER_VERSION:-v0.0.0-template}"
...
if [ -d "$INSTALL_DIR/.git" ]; then
    echo -e "${YELLOW}AdaptiveLearner is already installed in ${INSTALL_DIR}${NC}"
...
echo -e "${BLUE}Downloading AdaptiveLearner ${VERSION}...${NC}"
...
if [ ! -f .env ]; then
    ...
    SECRET=$(python3 -c "import secrets; print(secrets.token_hex(32))" 2>/dev/null || \
             ...
             echo "adaptive-learner-$(date +%s)-random")
...
# --- Build and start ---
echo ""
echo -e "${BLUE}Building and starting AdaptiveLearner (this may take a few minutes the first time)...${NC}"
echo ""
