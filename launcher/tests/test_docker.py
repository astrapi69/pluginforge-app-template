...
class TestDockerInstalled:
    ...
    def test_true_on_success(self) -> None:
        with patch("adaptive_learner_launcher.docker._run", return_value=_run_result(stdout="Docker version 27.1")):
    ...
    def test_false_on_nonzero_exit(self) -> None:
        with patch("adaptive_learner_launcher.docker._run", return_value=_run_result(returncode=1, stderr="boom")):
            ok, detail = docker.docker_installed()
        assert ok is False
...
class TestDockerDaemonRunning:
    ...
    def test_true_when_info_succeeds(self) -> None:
        with patch("adaptive_learner_launcher.docker._run", return_value=_run_result(stdout="Server: ...")):
    ...
    def test_timeout_surfaces_as_user_message(self) -> None:
        with patch("adaptive_learner_launcher.docker._run", side_effect=subprocess.TimeoutExpired(cmd="docker info", timeout=15)):
            ok, detail = docker.docker_daemon_running()
        assert ok is False
...
class TestComposeLogsTail:
    ...
    def test_returns_stdout_by_default(self, tmp_path: Path) -> None:
        with patch("adaptive_learner_launcher.docker._run", return_value=_run_result(stdout="log lines\n")):
    ...
    def test_falls_back_to_stderr_when_stdout_empty(self, tmp_path: Path) -> None:
        with patch("adaptive_learner_launcher.docker._run", return_value=_run_result(stderr="warning text")):
...
class TestRemoveVolumes:
    ...
    def test_removes_found_volumes(self) -> None:
        ls_result = _run_result(stdout="adaptive_learner_data\nadaptive_learner_cache\n")
        rm_result = _run_result()
        with patch("adaptive_learner_launcher.docker._run", side_effect=[ls_result, rm_result]):
