...
3. Inspect:
   ```bash
   unzip -l ~/Downloads/adaptive-learner-backup-*.bgb | grep -E "articles|books|manifest"
...
3. Inspect:
   ```bash
   unzip -l ~/Downloads/adaptive-learner-backup-*.bgb
   ```
...
{"format": "adaptive-learner-backup", "version": "9.9", "book_count": 0, "article_count": 0}
EOF
