...
**What format is used internally?** AdaptiveLearner stores chapter content as TipTap JSON. During export, this is converted to Markdown and then to the target format.
...
**Does AdaptiveLearner work offline?** Yes. AdaptiveLearner uses SQLite and stores everything locally. Only plugins that access external APIs (Grammar, Translation, Audiobook with cloud engines, AI with cloud providers) require an internet connection. Fonts are bundled locally.
...
**What happens if I close the browser without saving?** AdaptiveLearner saves automatically as you type (debounced). Additionally, unsaved changes are stored locally in your browser (IndexedDB). If you reopen a chapter with unsaved changes, you will be offered to restore them.
