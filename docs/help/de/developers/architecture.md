...
Diese Seite ist eine destillierte Sicht von außen auf die Struktur von AdaptiveLearner. Die interne Quelle der Wahrheit liegt in [`.claude/rules/architecture.md`](https://github.com/astrapi69/adaptive_learner/blob/main/.claude/rules/architecture.md) und in den README-Dateien pro Komponente; diese Seite hebt heraus, was externe Mitwirkende wissen müssen, um sich im Repo zurechtzufinden, ohne jede interne Regel lesen zu müssen.
...
| [astrapi69/adaptive_learner](https://github.com/astrapi69/adaptive_learner) | Dieses Repo. Buch- und Artikel-Autorenplattform. Pinnt `pluginforge ^0.5.0`. | MIT |

PluginForge-Änderungen sind eine eigene Codebase und ein eigener Release. PluginForge **nicht** aus AdaptiveLearner heraus editieren — PR gegen das PluginForge-Repo öffnen, nach dem Release den Pin hier hochziehen.
...
backend/app/
  ...
  exceptions.py          # AdaptiveLearnerError-Hierarchie
...
- **Services werfen `AdaptiveLearnerError`-Unterklassen**, niemals `HTTPException`. Der globale Exception-Handler in `main.py` mappt jede Unterklasse auf einen HTTP-Status (`NotFoundError` → 404, `ValidationError` → 400, `ConflictError` → 409, `ExportError`/`PluginError` → 500, `ExternalServiceError` → 502).
...
- Plugin-Pakete: `adaptive-learner-plugin-{name}` (kebab-case). Inneres Paket: `adaptive_learner_{name}` (snake_case).
...
1. Umgebungsvariable `ADAPTIVE_LEARNER_DATA_DIR` (höchste Priorität — Tests, Docker, Admin-Override)
...
- Eine `.adaptive-learner-production`-Marker-Datei, die der FastAPI-Lifespan schreibt. Sieht ein Test diese Datei, bricht der gesamte Testlauf mit `pytest.exit(returncode=2)` ab.
- `ADAPTIVE_LEARNER_TEST=1` + `TEST_DATABASE_URL=sqlite:///:memory:` werden vor dem ersten `app.*`-Import gesetzt.
...
Service        Wirft AdaptiveLearnerError-Unterklassen. Kennt kein HTTP.
Plugin         Wirft PluginError(plugin_name, message).
Extern         ExternalServiceError(service, message) für Pandoc/TTS/LanguageTool.
```

Jede Schicht behandelt nur, was sie kann; alles andere fließt nach oben. Der globale Exception-Handler in `backend/app/main.py` mappt `AdaptiveLearnerError`-Unterklassen auf HTTP-Status, fügt im Debug-Modus (`ADAPTIVE_LEARNER_DEBUG=true`) den Stacktrace in die Antwort ein und loggt alles ≥ 500 mit `exc_info=True`.
...
- [pluginforge](https://github.com/astrapi69/pluginforge) — das Plugin-Framework (PyPI). AdaptiveLearner-agnostisch, MIT.
