...
Der Linux-Launcher ist ein `adaptive-learner-launcher-linux`-ELF-Binary, das AdaptiveLearner per Klick startet: keine `docker compose`-Kommandos, keine offenen Terminal-Sitzungen. Docker führt die Anwendung weiterhin aus, der Launcher startet und stoppt sie nur für dich.
...
Den vollständigen Überblick inklusive Abschnitt „Ist Docker sicher zu installieren?" findest du in der [AdaptiveLearner-Anleitung zur Docker-Installation](install/docker-desktop.md). Auf Linux sind zwei Wege üblich:
...
- `adaptive-learner-launcher-linux`
- `adaptive-learner-launcher-linux.sha256`
...
```bash
sha256sum adaptive-learner-launcher-linux
cat adaptive-learner-launcher-linux.sha256
```

Der Hash von `sha256sum` muss mit der Hex-Zeichenkette in der `.sha256`-Datei übereinstimmen. Wenn nicht, das Binary **nicht** ausführen und das Ganze auf [GitHub Issues](https://github.com/astrapi69/adaptive_learner/issues) melden.

### 4. Launcher ausführbar machen

```bash
chmod +x adaptive-learner-launcher-linux
```
...
2. **AdaptiveLearner-Prüfung.** Der Launcher sucht eine bestehende AdaptiveLearner-Installation über die Manifest-Datei (`~/.config/adaptive_learner/install.json`) oder, auf einem frischen Rechner, am Standardort `~/adaptive_learner`.
...
In der Launcher-Leiste auf **AdaptiveLearner stoppen** klicken oder das Fenster einfach schließen. Der Launcher führt `docker compose down` aus und beendet sich. Docker läuft im Hintergrund weiter; nur die AdaptiveLearner-Container werden gestoppt.

## Zweiter Start

Launcher erneut starten. Wenn AdaptiveLearner bereits läuft (etwa weil das Fenster minimiert war), erkennt der Launcher die laufende Instanz und öffnet einfach den Browser auf der richtigen URL, ohne eine zweite Kopie zu starten.
...
```bash
docker compose -f ~/adaptive_learner/docker-compose.prod.yml logs --tail=100
```

**"./adaptive-learner-launcher-linux: cannot execute: required file not found"**
Das Binary braucht glibc 2.35 oder neuer. Du bist auf einer älteren Distribution. Die Distribution aktualisieren oder AdaptiveLearner stattdessen über `install.sh` aus dem Repository installieren.
...
**Aktivitätslog**
Jeder Start schreibt nach `~/.config/adaptive_learner/install.log` (1 MB Rotation, 1 Backup). Diese Datei Fehlerberichten beilegen. Siehe Abschnitt [Aktivitätslog](#aktivitatslog) für Details.
...
```
~/.config/adaptive_learner/install.log
```
