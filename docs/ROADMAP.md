...
Latest release: v0.30.0 (launcher localized in 8 languages with full parity-test enforcement; DEP-DBPATH-01 cycle closes — ADAPTIVE_LEARNER_DB_PATH no longer honoured as a path override, warning-only on lingering env var; 5 new bilingual core help pages — books bulk-export, cross-platform installers, architecture, contributing, deployment, API reference; plugin dev guide refreshed for Vite 8 + Node 24; pre-release dependency sweep with fastapi 0.135 → 0.136 lock-step + in-range patches across all subsystems).
Open tasks: 1 P2 (PB-PHASE4) + 3 active (P3..P5) + 2 BLOCKED-on-upstream + 1 P5 (LAUNCHER-I18N-NATIVE-REVIEW-01, public call-for-reviewers at [#18](https://github.com/astrapi69/adaptive_learner/issues/18))
...
- [ ] **PB-PHASE4**: Picture-Book plugin (kinderbuch) — Sessions
  ...
  - Plugin separation: `adaptive-learner-plugin-kinderbuch` owns
    `picture_book` exclusively. A separate
    `adaptive-learner-plugin-comics` will own `comic_book` once
