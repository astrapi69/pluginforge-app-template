...
**Status in this template:** **shipped.** The 8 backend YAML catalogs under `backend/config/i18n/` (de, el, en, es, fr, ja, pt, tr) are the single source of truth; `scripts/sync_i18n.py` (+ `make sync-i18n` / `sync-i18n-check`) mirrors them to `frontend/src/data/i18n/*.json`, and `hooks/useI18n.ts` loads the bundled JSON via `import.meta.glob` (default language eager, the rest lazy chunks) instead of fetching `api.i18n.get(lang)` — so i18n needs no backend roundtrip and works in a static/offline build. The committed JSON is regenerated by `make sync-i18n`; `make sync-i18n-check` is the drift gate, and `frontend/src/hooks/i18n-bundle.test.ts` pins that the bundle is populated.
