...
location (``~/.local/share/adaptive_learner`` on Linux/macOS,
``%LOCALAPPDATA%\\adaptive_learner`` on Windows).
...
- **Breadcrumb at old path.** A ``.migrated-YYYY-MM-DD`` file is
  ...
  deleting.
- **Skipped in test mode** (``ADAPTIVE_LEARNER_TEST=1``). Tests get a
...
_LEGACY_DB = _PROJECT_BACKEND_DIR / "adaptive_learner.db"
...
def _legacy_paths(target: Path) -> list[tuple[str, Path, Path]]:
    ...
    return [
        ("adaptive_learner.db", _LEGACY_DB, target / "adaptive_learner.db"),
...
def migrate_data_dir_if_needed() -> None:
    ...
    for label, legacy, dst in items:
        ...
        if dst.exists():
            raise RuntimeError(
                ...
                f"then restart AdaptiveLearner."
            )
        ...
        try:
            breadcrumb.write_text(
                f"AdaptiveLearner data moved to {dst} on {date.today().isoformat()}.\n"
    ...
    marker.touch()
    logger.warning(
        "AdaptiveLearner data migrated to %s. Verify and delete legacy "
