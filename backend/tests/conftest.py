...
CRITICAL: this module sets ADAPTIVE_LEARNER_TEST=1, TEST_DATABASE_URL and
ADAPTIVE_LEARNER_DATA_DIR BEFORE any ``app.*`` import. Order matters:

- ``app/database.py`` reads ``ADAPTIVE_LEARNER_TEST`` / ``TEST_DATABASE_URL``
...
- ``app/paths.py`` reads ``ADAPTIVE_LEARNER_DATA_DIR`` lazily via
...
if a test ever points ``ADAPTIVE_LEARNER_DATA_DIR`` at a path that contains
real data.
"""
...
#     mismatching ``HookspecMarker("adaptive_learner.plugins")`` and
...
def _verify_test_isolation() -> None:
    ...
    1. DB: engine URL must not contain ``adaptive_learner.db``.
    2. Filesystem: the resolved data directory must not contain a
       ``.adaptive-learner-production`` marker (written by the FastAPI
    ...
    if marker.exists():
        pytest.exit(
            ...
            "Fix:\n"
            "  - Set ADAPTIVE_LEARNER_DATA_DIR explicitly to a test path,\n"
